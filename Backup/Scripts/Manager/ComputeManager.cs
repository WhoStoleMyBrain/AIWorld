...
public class ComputeManager : MonoBehaviour
{
    ...
    {
        int kernelFillArray = noiseShader.FindKernel("FillArray");
        ...
        noiseShader.SetBuffer(kernelFillArray, "specialBlocksBuffer", noiseBuffer.specialBlocksBuffer);
        noiseShader.SetBuffer(kernelFillArray, "voxelArray", noiseBuffer.noiseBuffer);
        noiseShader.SetBuffer(kernelFillArray, "count", noiseBuffer.countBuffer);

        noiseShader.SetVector("chunkPosition", cont.chunkPosition);
        noiseShader.SetVector("seedOffset", Vector3.zero);

        noiseShader.Dispatch(kernelFillArray, xThreads, yThreads, xThreads);
    ...
    {
        int kernelCalculateVertices = voxelShader.FindKernel("CalculateVertices");
        int kernelSumNormals = voxelShader.FindKernel("SumNormals");
        int kernelGenerateFaces = voxelShader.FindKernel("GenerateFaces");
        meshBuffer.countBuffer.SetData(new uint[] { 0, 0, 0 });
        voxelShader.SetVector("chunkPosition", pos);
        voxelShader.SetBuffer(kernelCalculateVertices, "voxelArray", meshBuffer.modifiedNoiseBuffer);
        voxelShader.SetBuffer(kernelCalculateVertices, "counter", meshBuffer.countBuffer);
        voxelShader.SetBuffer(kernelCalculateVertices, "cellVertices", meshBuffer.cellVerticesBuffer);
        voxelShader.Dispatch(kernelCalculateVertices, xThreads, yThreads, xThreads);

        voxelShader.SetBuffer(kernelGenerateFaces, "voxelArray", meshBuffer.modifiedNoiseBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "cellVertices", meshBuffer.cellVerticesBuffer);
        voxelShader.Dispatch(kernelGenerateFaces, xThreads, yThreads, xThreads);

        voxelShader.SetBuffer(kernelGenerateFaces, "voxelArray", meshBuffer.modifiedNoiseBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "counter", meshBuffer.countBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "vertexBuffer", meshBuffer.vertexBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "normalBuffer", meshBuffer.normalBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "cellVertices", meshBuffer.cellVerticesBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "colorBuffer", meshBuffer.colorBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "indexBuffer", meshBuffer.indexBuffer);
        voxelShader.SetBuffer(kernelGenerateFaces, "transparentIndexBuffer", meshBuffer.transparentIndexBuffer);
        voxelShader.Dispatch(kernelGenerateFaces, xThreads, yThreads, xThreads);
